...
無料枠にはスリープ、月間稼働時間などの制限があります。詳細は [Render の無料枠ドキュメント](https://render.com/docs/free) を参照してください。

本アプリケーション利用時の注意点：

| 注意点 | 対策 |
|--------|------|
| 無操作でスリープ状態になる | イベント開始前にアクセスしてウォームアップ |
| スリープからの復帰に時間がかかる | 事前にセッションを作成しておく |
| サービス再起動でセッションデータは消失 | イベント中の再デプロイは避ける |
